># Hunter Radar V1.4 — OpenAPI Freeze v1.4

> **🧊 Freeze 时点:2026-06-15(W1 末,M5 启动日)**
> 配套:本仓库后端 `app/api/*.py` 共 **27 个端点**(6 router + 2 health)
> 配套:本仓库后端 DTO **24 个**(`*DTO` / `*Request` / `*Summary`)
...
curl http://localhost:8000/openapi.json > docs/openapi-frozen-v1.4.json
# 或用 Python 一次性导出:
uv run python -c "from app.main import app; import json; print(json.dumps(app.openapi(), indent=2))" > docs/openapi-frozen-v1.4.json
```

> **沙箱 fallback**(无 PG / Redis / asyncpg / sentry_sdk):
> 本仓库 `docs/openapi-frozen-v1.4.json` 由 `scripts/m5t1_dump_openapi.py` 静态扫描
...
5. 重 freeze:跑 `uv run python -c "from app.main import app; import json; print(json.dumps(app.openapi(), indent=2))" > docs/openapi-frozen-v1.4.json`,人读版同步更新 + 顶部冻结时点更新
6. 同步 FE-010:见 §九
