...
import { ThreatHistoryChart, type ThreatHistoryPoint } from "@/components/radar/ThreatHistoryChart";
import { SkeletonCard } from "@/components/common/Skeleton";
import { toast } from "@/components/common/Toast";
import { threatScoreColor } from "@/lib/design-tokens";

export const Route = createLazyRoute("/basket")({
...
// ─── Basket Snapshot History (P3-01: aggregate member EMA by date) ──

function useBasketSnapshotHistory(tickers: string[], days = 90) {
  return useQuery({
    queryKey: ["basket-snapshot-history", tickers, days],
    queryFn: async () => {
      const histories = await Promise.all(
        tickers.map(async (tk) => {
          try {
            return await api.getThreatHistory(tk, days);
          } catch {
            return [] as Array<{ date: string; total: number; total_raw: number }>;
          }
        }),
      );
      // Aggregate: cross-member mean per date
      const byDate = new Map<string, { sum: number; count: number }>();
      for (const hist of histories) {
        for (const p of hist) {
          const entry = byDate.get(p.date) ?? { sum: 0, count: 0 };
          entry.sum += p.total;
          entry.count += 1;
      ...
      return [...byDate.entries()]
        ...
        .map(([date, { sum, count }]): ThreatHistoryPoint => ({
          date,
          total: sum / count,
          total_raw: sum / count,
        }));
...
function BasketDetailView({
  ...
  // P3-01: Snapshot history (date-over-date aggregate line chart)
  const { data: snapshotHistory } = useBasketSnapshotHistory(tickers, 90);

  // P3-01: Aggregate danger score (mean / max)
  const aggregate = useMemo(() => {
    if (!threats) return null;
    const scores = Object.values(threats).map((th) => th.total);
  ...
  const renameMutation = useMutation({
    mutationFn: (name: string) => api.updateBasket(basketId, { name }),
    onSuccess: () => {
      setRenaming(false);
      toast.success(t("basket.renamed"));
      queryClient.invalidateQueries({ queryKey: ["basket-detail", basketId] });
  ...
  const exportCsv = useCallback(() => {
    if (!data) return;
    const header = ["ticker", "threat_score", "options", "short", "divergence", "insider", "lifecycle"];
    const rows = data.members.map((m) => {
      const th = threats?.[m.ticker];
      return [
        m.ticker,
        th ? th.total.toFixed(1) : "",
        th ? th.module_options.toFixed(1) : "",
        th ? th.module_short.toFixed(1) : "",
        th ? th.module_divergence.toFixed(1) : "",
        th ? th.module_insider.toFixed(1) : "",
        th?.signal_lifecycle ?? "",
      ].join(",");
    });
    ...
    a.href = url;
    a.download = `${data.basket.name.replace(/[^\w\u4e00-\u9fa5]+/g, "_")}_members.csv`;
    a.click();
  ...
  return (
    <div className="space-y-4">
      {/* Header (PRD §3.5: name + member count + aggregate danger score) */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          <button onClick={onBack} className="text-sm text-slate-400 hover:text-slate-200 focus:outline-none focus:ring-1 focus:ring-slate-400 rounded">
            ← {t("basket.backToList")}
          </button>

          {/* Name + inline rename (P3-01) */}
          {renaming ? (
            <div className="flex items-center gap-2 mt-1">
              <input
                ...
                className="px-2 py-0.5 bg-slate-800 rounded text-lg font-bold text-slate-100 w-56"
                aria-label={t("basket.rename")}
              />
              <button
                onClick={() => newName.trim() && renameMutation.mutate(newName.trim())}
                disabled={renameMutation.isPending}
                className="px-2 py-1 text-xs rounded bg-hunter-red text-white disabled:opacity-50"
              >
                {t("common.confirm")}
              </button>
              <button onClick={() => setRenaming(false)} className="px-2 py-1 text-xs rounded bg-slate-700 text-slate-200">
                {t("basket.cancel")}
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2 mt-1">
              <h1 className="text-2xl font-bold truncate">{basket.name}</h1>
              <button
                onClick={() => { setNewName(basket.name); setRenaming(true); }}
                className="text-xs text-slate-500 hover:text-slate-300 px-1.5 py-0.5 rounded hover:bg-slate-800 transition-colors focus:outline-none focus:ring-1 focus:ring-slate-400"
                aria-label={t("basket.rename")}
                title={t("basket.rename")}
              >
                ✏️
              </button>
            </div>
          ...
          {/* Aggregate danger score (P3-01) */}
          <div className="flex items-center gap-4 mt-2 text-sm flex-wrap">
            <span className="text-slate-400">
              {members.length} {t("basket.members")}
            </span>
            {aggregate && (
              <>
                <span className="text-slate-400">
                  {t("basket.aggregate.mean")}{" "}
                  <strong className="font-mono" style={{ color: threatScoreColor(aggregate.mean) }}>
                    {aggregate.mean.toFixed(1)}
                  </strong>
                </span>
                <span className="text-slate-400">
                  {t("basket.aggregate.max")}{" "}
                  <strong className="font-mono" style={{ color: threatScoreColor(aggregate.max) }}>
                    {aggregate.max.toFixed(1)}
                  </strong>
                </span>
              </>
            )}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={exportCsv}
            className="px-3 py-1.5 text-sm rounded bg-slate-700 text-slate-200 hover:bg-slate-600 transition-colors focus:outline-none focus:ring-1 focus:ring-slate-400"
          >
            ⬇ {t("basket.exportCsv")}
          </button>
          <button onClick={deleteBasket} className="px-3 py-1.5 text-sm rounded bg-red-900/40 text-red-300 border border-red-800/50 hover:bg-red-900/60 hover:text-red-200 transition-colors focus:outline-none focus:ring-1 focus:ring-red-400">
      ...
      {/* P3-01: Snapshot History (PRD §3.5 date-over-date line chart) */}
      <section>
        <h2 className="text-lg font-semibold mb-2">{t("basket.snapshotHistory.title")}</h2>
        <div className="overflow-x-auto">
          {snapshotHistory && snapshotHistory.length >= 2 ? (
            ...
            <div className="text-xs text-slate-500 bg-slate-900/50 rounded p-4">
              {t("basket.snapshotHistory.noData")}
            </div>
          )}
        </div>
      </section>
